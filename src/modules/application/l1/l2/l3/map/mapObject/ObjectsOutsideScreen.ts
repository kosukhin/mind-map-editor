...
export class ObjectsOutsideScreen {
  ...
  public count<R extends GuestType<CountDocument>>(config: ObjectsConfig, guest: R) {
    ...
    chain.result(
      this.factories.guestInTheMiddle.create(guest, ({ objects, layer, position }: ChainDocument) => {
        const multiplier = directionMultiplier[config.direction];
        const sortedObjects = objects.sort((a, b) => a.position[axisToPositionIndex[config.axis]] * multiplier - b.position[axisToPositionIndex[config.axis]] * multiplier);
        const nearest = sortedObjects.filter((object) => {
          const objectPoint = object.position[axisToPositionIndex[config.axis]] + (isPositiveDirection ? 0 : object[axisToSize[config.axis]]);
          const screenPoint = position[config.axis] * -1 + (isPositiveDirection ? layer[axisToSize[config.axis]]() : 0);

          localDebug('mb nearest points', config.direction, 'objectP=', objectPoint, 'screenP=', screenPoint);
