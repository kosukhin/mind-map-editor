import { SourceEmpty, GuestObjectType, FactoryType, GuestAwareAllType } from 'patron-oop';
import {
  MapDocument,
  MapObjectDocument,
...
export class MapObjectsVisible implements MapObjectsType {
  ...
  public constructor(
    ...
    canvas.size(factories.patron.create(chain.guestKey('size')));
    layerDep.position(factories.patron.create(chain.guestKey('position')));
    mapFile.currentMap(factories.patron.create(chain.guestKey('map')));
    chain.value(
